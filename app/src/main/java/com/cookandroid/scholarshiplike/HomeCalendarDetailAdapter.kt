...
class HomeCalendarDetailAdapter(val context: Context, val calendarLayout: LinearLayout, val date: Date, val pageindex: Int) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CalendarItemHolder {
        val view =
            LayoutInflater.from(context).inflate(R.layout.fragment_home_calendar_item_list, parent, false)
    ...
    inner class CalendarItemHolder(itemView: View?) : RecyclerView.ViewHolder(itemView!!) {
        ...
        private val itemCalendarContents: LinearLayout = itemView!!.item_calendar_contents //해당 날짜 밑 리니어 레이아웃
        ...
        fun bind(data: Int, position: Int, context: Context) {
            ...
            if (position < firstDateIndex || position > lastDateIndex) {
                itemCalendarDateText.setTextColor(Color.parseColor("#00000000"))
            }
            ...
            fun showPopup(date:CharSequence) {
                val view =
                    LayoutInflater.from(context).inflate(R.layout.fragment_home_calendar_popup, null, false)
